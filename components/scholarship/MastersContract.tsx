...
export const MastersContract: FC<TContract> = ({ scholarship }) => {
  ...
  const updateMastersContractMutation = useMutation({
    mutationFn: (values: {
        ...
        onSubmit={async (values, actions) => {
          {
            if (studentSignature && guardianSignature) {
              ...
              await updateMastersContractMutation.mutateAsync(dataToCall);
            } else {
              toast.error(
                t("missingSignatures") ??
                  "Please provide both applicant and guardian signatures"
              );
            }
